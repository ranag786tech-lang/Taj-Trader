...
function Header() {
  ...
  return <>
    <header className="site-header">
      ...
      <div className="header-actions"><a className="header-phone" href="tel:"><Phone size={16} /> Call store</a><a className="button button-small" href="#quote">Get a quote <ArrowRight size={16} /></a><button className="menu-button" onClick={() => setOpen(!open)} aria-label={open ? 'Close menu' : 'Open menu'}>{open ? <X /> : <Menu />}</button></div>
    </header>
    {open && <div className="mobile-menu"><a href="#products" onClick={() => setOpen(false)}>Products</a><a href="#brands" onClick={() => setOpen(false)}>Brands</a><a href="#services" onClick={() => setOpen(false)}>Services</a><a href="#about" onClick={() => setOpen(false)}>About</a><a href="#contact" onClick={() => setOpen(false)}>Contact</a></div>}
  </>
}
